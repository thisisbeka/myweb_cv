...
function ContactCard({ icon, label, value, href, color }: ContactCardProps) {
  return (
    ...
    >
      ...
      <div className="text-xs md:text-sm uppercase tracking-wider text-gray-300 mb-2">{label}</div>
      <div className="text-[10px] md:text-xs text-gray-500 font-medium break-words group-hover:text-gray-300 transition-colors duration-300">
        {value}
      </div>
    </a>
  );
